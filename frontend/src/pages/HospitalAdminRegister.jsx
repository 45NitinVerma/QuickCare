...
import {
  ...
  Mail, Phone, Lock, Globe, MapPin, FileText, Upload, Shield,
...
const HOSPITAL_TYPES = ['Private', 'Government', 'Clinic', 'Multi-speciality'];
const INDIAN_STATES = [
...
const selectStyle = {
  ...
  padding: '0 0.75rem', fontSize: '0.875rem', border: '1px solid var(--border)',
  outline: 'none',
};
      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <React.Fragment key={i}>
            ...
            {i < STEPS.length - 1 && (
              <div className="w-8 sm:w-12 h-0.5 mb-4 transition-all duration-300" style={{ background: i < current ? 'var(--primary)' : 'var(--border)' }} />
            )}
          </React.Fragment>
        );
      })}
...
export function HospitalAdminRegister() {
  ...
  const [form, setForm] = useState({
    ...
    hospitalName: '', regNumber: '', hospitalType: '', state: '', city: '',
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!/^\+?\d{10,13}$/.test(form.contactNumber.replace(/\s/g, ''))) e.contactNumber = 'Valid contact number';
  ...
  const handleVerify = () => {
    ...
    setTimeout(() => {
      register('Admin', {
        ...
        hospitalInfo: {
          name: form.hospitalName, registrationNumber: form.regNumber,
          type: form.hospitalType, state: form.state, city: form.city,
          address: form.address, pincode: form.pincode,
          contactNumber: form.contactNumber, email: form.hospitalEmail,
          website: form.website, gst: form.gst, pan: form.pan,
          certFileName: uploadedFile?.name || 'certificate.pdf',
        ...
        <div className="w-full max-w-2xl">
          ...
          >
                ...
                {step === 0 && (
                  <motion.div key="s0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <FieldWrap label="Official Email *">
                        <TextInput icon={Mail} type="email" placeholder="admin@hospital.com" value={form.email} onChange={e => set('email', e.target.value)} />
                        {err('email')}
                      </FieldWrap>
                  ...
                  <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2">
                        <FieldWrap label="Hospital Name *">
                          <TextInput icon={Building2} placeholder="Apollo QuickCare Hospital" value={form.hospitalName} onChange={e => set('hospitalName', e.target.value)} />
                          {err('hospitalName')}
                        </FieldWrap>
                      </div>
                      <FieldWrap label="Registration Number *">
                        <TextInput icon={FileText} placeholder="MH/HOSP/2018/04821" value={form.regNumber} onChange={e => set('regNumber', e.target.value)} />
                        {err('regNumber')}
                      </FieldWrap>
                      <FieldWrap label="Hospital Type *">
                        <select style={selectStyle} value={form.hospitalType} onChange={e => set('hospitalType', e.target.value)}>
                      ...
                      <FieldWrap label="City *">
                        <TextInput icon={MapPin} placeholder="Mumbai" value={form.city} onChange={e => set('city', e.target.value)} />
                        {err('city')}
                      </FieldWrap>
                      <div className="sm:col-span-2">
                        <FieldWrap label="Full Address *">
                          <TextInput icon={MapPin} placeholder="14, Business Park, Area, City" value={form.address} onChange={e => set('address', e.target.value)} />
                          {err('address')}
                        </FieldWrap>
                      </div>
                      <FieldWrap label="Pincode *">
                        <TextInput placeholder="400076" maxLength={6} value={form.pincode} onChange={e => set('pincode', e.target.value)} />
                        {err('pincode')}
                      </FieldWrap>
                      <FieldWrap label="Contact Number *">
                        <TextInput icon={Phone} type="tel" placeholder="+91-22-6780-0000" value={form.contactNumber} onChange={e => set('contactNumber', e.target.value)} />
                        {err('contactNumber')}
                      </FieldWrap>
                      <FieldWrap label="Hospital Official Email *">
                        <TextInput icon={Mail} type="email" placeholder="info@hospital.com" value={form.hospitalEmail} onChange={e => set('hospitalEmail', e.target.value)} />
                        {err('hospitalEmail')}
                      </FieldWrap>
                      <FieldWrap label="Website (optional)">
                        <TextInput icon={Globe} placeholder="www.hospital.com" value={form.website} onChange={e => set('website', e.target.value)} />
                      </FieldWrap>
                    </div>
                  </motion.div>
                )}
                  ...
                  <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                    ...
                    <div>
                      ...
                      <div
                        ...
                        style={{
                          borderColor: dragging ? 'var(--primary)' : uploadedFile ? 'var(--success)' : 'var(--border)',
                          background: dragging ? 'var(--primary-muted)' : uploadedFile ? 'var(--success-light)' : 'var(--bg-secondary)',
                        }}
                      >
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <FieldWrap label="GST Number (optional)">
                        <TextInput placeholder="27AACCA0001B1ZQ" value={form.gst} onChange={e => set('gst', e.target.value)} />
                      </FieldWrap>
                      <FieldWrap label="PAN Number (optional)">
                        <TextInput placeholder="AACCA0001B" value={form.pan} onChange={e => set('pan', e.target.value)} />
                      </FieldWrap>
                    </div>
                    <div className="flex items-start gap-3 p-4 rounded-2xl border" style={{ background: 'var(--info-light)', borderColor: 'rgba(59,130,246,0.2)' }}>
                      ...
                      <p className="text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                        Your documents are encrypted and stored securely. They will only be used for hospital verification purposes and are reviewed by the QuickCare compliance team.
                      </p>
                    </div>
                  </motion.div>
                )}
                  ...
                  <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                    ...
                    {!otpSent ? (
                      ...
                      <>
                        ...
                        <div className="text-center">
                          {canResend ? (
                            <button onClick={sendOtp} className="text-sm font-semibold hover:underline" style={{ color: 'var(--primary)' }}>
                              Resend OTP
                            </button>
                          ) : (
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" checked={termsAccepted} onChange={e => setTermsAccepted(e.target.checked)}
                        className="w-4 h-4 mt-0.5 rounded accent-[var(--primary)]" required />
                      <span className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                        I accept the <span className="font-semibold hover:underline cursor-pointer" style={{ color: 'var(--primary)' }}>Terms & Conditions</span> and <span className="font-semibold hover:underline cursor-pointer" style={{ color: 'var(--primary)' }}>Privacy Policy</span>. I certify that all provided information is accurate and authentic.
                      </span>
                    </label>
                    {err('terms')}
                  </motion.div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: 'var(--border)', background: 'var(--bg-secondary)' }}>
              ...
              {step < STEPS.length - 1 ? (
                <button onClick={next} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90"
